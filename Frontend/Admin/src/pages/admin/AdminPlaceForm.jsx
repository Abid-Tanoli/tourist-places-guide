...
const AdminPlaceForm = () => {
    ...
    const fetchMeta = async () => {
      try {
        const [catRes, regRes] = await Promise.all([
          ...
          api.get("/regions?status=active"),
        ]);
        setCategories(Array.isArray(catRes.data) ? catRes.data : []);
        setRegions(Array.isArray(regRes.data) ? regRes.data : []);
      } catch {
        // Non-critical — form still works with free-text fallback
      }
    };
    fetchMeta();
  }, []);
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      setError(getApiError(requestError, "Unable to create category."));
    } finally {
  ...
  const handleAddRegion = async () => {
    ...
    try {
      const { data } = await api.post("/regions", { name: newRegion.trim() });
      setRegions((prev) => [...prev, data].sort((a, b) => a.name.localeCompare(b.name)));
      setForm((prev) => ({ ...prev, region: data.name }));
      setNewRegion("");
    } catch (requestError) {
      setError(getApiError(requestError, "Unable to create region."));
    } finally {
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <label className="block">
              ...
              <select name="region" value={form.region} onChange={handleChange} className="w-full border rounded-md px-3 py-2" required>
              ...
              <div className="mt-1.5 flex gap-2">
                ...
                />
                <button type="button" onClick={handleAddRegion} disabled={addingRegion || !newRegion.trim()} className="px-2 py-1 text-xs rounded bg-blue-600 text-white hover:bg-blue-700 disabled:bg-gray-400">
                  {addingRegion ? "..." : "+ Add"}
                </button>
              </div>
            </label>
            <label className="block">
              ...
              <select name="category" value={form.category} onChange={handleChange} className="w-full border rounded-md px-3 py-2" required>
              ...
              <div className="mt-1.5 flex gap-2">
                <input
                  ...
                  className="flex-1 border rounded-md px-2 py-1 text-xs"
                />
                <button type="button" onClick={handleAddCategory} disabled={addingCategory || !newCategory.trim()} className="px-2 py-1 text-xs rounded bg-blue-600 text-white hover:bg-blue-700 disabled:bg-gray-400">
                  {addingCategory ? "..." : "+ Add"}
                </button>
              </div>
            </label>
